    ...
    <div class="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
        ...
        <div>
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-50">Browse Items</h2>
                    <p class="mt-1 text-slate-500 dark:text-slate-400">Browse available items according to your needs.</p>
                </div>
                        ...
                        <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-700 dark:bg-slate-900 dark:shadow-slate-900/50 dark:hover:shadow-slate-900/70">
                            <div class="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->image_path): ?>
                                    <img src="<?php echo e(asset('storage/' . $item->image_path)); ?>" alt="<?php echo e($item->name); ?>" class="h-full w-full object-cover transition duration-300 group-hover:scale-110">
                                <?php else: ?>
                            ...
                            <div class="space-y-3 p-4">
                                <div>
                                    <h3 class="line-clamp-1 text-sm font-bold text-slate-900 dark:text-slate-50"><?php echo e($item->name); ?></h3>
                                    <p class="mt-1 line-clamp-2 text-xs text-slate-500 dark:text-slate-400"><?php echo e($item->description ?: 'No description provided.'); ?></p>
                                </div>

                                <div class="flex items-end justify-between border-t border-slate-200 pt-2 dark:border-slate-700">
                                    <div>
                                        <div class="bg-gradient-to-r from-blue-600 to-violet-600 bg-clip-text text-lg font-bold text-transparent">&#8369;<?php echo e(number_format($item->price, 2)); ?></div>
                                    ...
                                    <div class="text-right text-[11px] text-slate-400 dark:text-slate-500">
                                        <div><?php echo e($item->categoryRecord?->name ?? ucfirst(str_replace('-', ' ', $item->category ?? 'Other'))); ?></div>
                                        <div class="font-medium"><?php echo e(ucfirst($item->status)); ?></div>
                                    </div>
                                </div>

                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->guard()->check()): ?>
                                    <a href="<?php echo e(route('item.view', $item->id)); ?>" class="block rounded-lg bg-gradient-to-r from-blue-600 to-violet-600 px-3 py-2.5 text-center text-xs font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:shadow-xl">
                                        View Details
                                    </a>
                                <?php else: ?>
                                    <a href="<?php echo e(route('login')); ?>" class="block rounded-lg bg-slate-900 px-3 py-2.5 text-center text-xs font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700">
                                        Sign In
                                    </a>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
...
<?php /**PATH C:\laragon\www\sub\resources\views/livewire/item-filter.blade.php ENDPATH**/ ?>